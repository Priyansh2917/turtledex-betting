...
class BallSpawnView(View):
    ...
    @classmethod
    async def get_random(cls, bot: "BallsDexBot", rare_only: bool = False):
        """
        Get a new instance with a random countryball. Rarity values are taken into account.
        If rare_only is True, only balls with rarity 0.01 to 1.5 are considered.
        """
        countryballs = list(filter(lambda m: m.enabled, balls.values()))
        if not countryballs:
            raise RuntimeError("No ball to spawn")
        if rare_only:
            countryballs = [m for m in countryballs if 0.01 <= m.rarity <= 1.5]
            if not countryballs:
                raise RuntimeError("No balls in the rare range (0.01-1.5) to spawn")
